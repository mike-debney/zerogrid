...
- **`sensor.zerogrid_available_load`** - Grid headroom available for new loads (in amps)
...
- **`sensor.zerogrid_controlled_load`** - Actual planned consumption of controlled loads (in amps)
...
- **`switch.zerogrid_enable_load_control`** - Master enable/disable for load control
- **`switch.zerogrid_allow_grid_import`** - Enable/disable grid import (useful for battery systems)
...
logger:
  default: info
  logs:
    homeassistant.components.zerogrid: debug
```
