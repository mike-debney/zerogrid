...
def test_total_load_is_capped_by_what_the_supply_can_deliver():
    ...
    assert cfg.max_total_load_amps == 40.0
...
def test_intervals_fall_back_to_defaults_when_not_configured():
    ...
    cfg = cfg.controllable_loads["Bare"]
...
def test_loads_removed_from_the_options_do_not_linger(make):
    ...
    assert set(cfg.controllable_loads) == {"Keep", "Drop"}

    zerogrid.parse_config(
        cfg, {**BASE, "controllable_loads": [load("Keep", "switch.keep", "sensor.keep")]}
    )
    assert set(cfg.controllable_loads) == {"Keep"}
...
def test_solar_consumption_follows_whether_a_solar_entity_is_set():
    ...
    zerogrid.parse_config(
        cfg,
        BASE)
    assert cfg.allow_solar_consumption is False

    cfg = Config()
    zerogrid.parse_config(
        cfg,
        {**BASE, "solar_generation_amps_entity": "sensor.solar"})
    assert cfg.allow_solar_consumption is True
...
def test_an_explicitly_zero_interval_is_kept():
    ...
    cfg = cfg.controllable_loads["Unlimited"]
